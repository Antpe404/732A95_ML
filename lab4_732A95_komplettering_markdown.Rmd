...
CV_ica(data=nir, folds=3, M=80)

```

It shows that only one component from the ICA analysis is a very sufficient explanatory variable to describe the \textit{Viscosity} in a linear regression. It has a very low CV score. From the plot I would choose two components though. The CV score gets almost zero already at two components, and thereafter gets even smaller for every component I add. From the elbow method I would choose two components though. That results in a minimized number of components and a very very low CV score.

Note that all code used in this complimentary assignment is already visualized, why I have no appendix.
