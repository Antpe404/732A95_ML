...
Finally, the test error is presented below. MENTION STUFF ABOUT 0/1, positive 1 means occurance tenderar att explaina 1. negativt 0 means ordens icke occurance explains att mailet is 0, not about Conference.
...
SVM<-function(sv=c(1), H=1, b=0, N=500, M, beta){
  ...
  plot(errorrate, main=paste(
"Errorrate as a function of \n number of training points,
  M=", M, ", beta=", beta,".", sep=""), type="o", las=1)
}
...
Klura lite paa visualiseringen. Antingen en plot dar alla visas i GG. Eller en rubrik, övergripande, samt bara M och beta i de fyra plottarna.

Have to think about whether beta -0.05 gives a smoother result than 0.

The call with M=20, beta=0 is the slowest because it generates the most computation. The higher the $\beta$ is the more iterations are true in step 5 of the pseudo code, thus the more computation needs to be done. That also applies for M=20 compared to M=500. When M=500, the $|S|$ never gets bigger than M when I run 500 training points. That means that the exclusion of the least important support vector in step eight in the pseodo code is never computed. 
